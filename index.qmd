---
title: "Quantifying drivers-overtaking-bicyclists with surrogate safety measures from a high-resolution 3D lidar"

authors: 
  - name: Rayane Moustafa
    email: rayanem@umich.edu
    affiliations: 
      - ref: um-dearborn
  - name: Gandhimathi Padmanaban
    email: gmathi@umich.edu
    affiliations: 
      - ref: um-dearborn
  - name: Fred Feng*
    email: fredfeng@umich.edu
    corresponding: true
    affiliations: 
      - ref: um-dearborn

affiliations:
  - id: um-dearborn
    name: University of Michigan-Dearborn
    department: Industrial and Manufacturing Systems Engineering
    address: 4901 Evergreen Rd
    city: Dearborn
    state: MI
    country: USA
    postal-code: 48128
    url: https://umdearborn.edu

date: 2026-01-14

footer: "Transportation Research Board Annual Meeting 2026"

---

## Bicycling in the U.S.

- Bicycling is underutilized with **0.5%** of commuters riding a bicycle (vs. 69.2% commuters driving alone).[@uscb2024commuting]
- **1,166** bicyclists were killed and **49,989** injured in 2023.[@ncsa2025bicyclists]
- Drivers overtaking bicyclists is one of the most dangerous encounters with rear-end collisions accounting for 40% bicyclist fatalities.[@lab2014bicyclist]
- Previous studies on overtaking bicyclists typically used lower resolution sensors (e.g., ultrasonic sensors [@beck2019much] or 2D lidar [@dozza2016drivers]), offering limited insights.

## High resolution 3D lidar

- Lidar (Light Detection and Ranging) is a laser-based sensor that captures 3D representations of surroundings.
- It enables detailed 3D analysis of road user behaviors and interactions (e.g., drivers, bicyclists) in real-world settings.

:::: {.columns}
::: {.column width="30%"}
![](assets/images/OS1-lidar.png){height=290px}
:::
::: {.column width="70%"}
![](assets/images/pcd-ouster.png){height=290px}
:::
::::

## Study objectives

- Collecting and sharing real-word bicycle riding data using high resolution lidar and cameras.
- Develop an algorithm to automatically detect and track overtaking vehicles based on the lidar point clouds.
- Develop quantitative surrogate safety measures for bicycling safety research.

## {data-menu-title="The lidar-equipped research bicycle"}

::: {style="text-align:center;font-size:50px;"}
![The research bicycle equipped with a lidar (Ouster OS1 128-channel)](assets/images/lidar-bike.png){height="550"}
:::

## {data-menu-title="Point cloud demo"}

::: {style="text-align:center;font-size:50px;"}
[![Demonstration of the point cloud collected from the lidar bicycle](assets/images/point-cloud-demo.png){height="550"}](https://fenggroup.org/pointcloud/examples/bike-2.html){preview-link="true"}
:::

## Preliminary data collection

:::: {.columns}
::: {.column width="58%"}
- Two trips in Ann Arbor, MI
- **4-lane** arterial road
  - **35 mph** speed limit
  - **painted bike lane** on each side
- Bicycle riding behavior
  - stay in the middle of bike lane
  - riding speed: 12-15 mph

- **306** vehicle overtaking events
:::
::: {.column width="42%"}
![](assets/images/ride-course.png)
![](assets/images/front-camera-fov.png)
:::
::::

## {data-menu-title="Data collection demo video"}

::: {style="text-align: center;"}
<iframe
  width="880"
  height="550"
  src="https://www.youtube.com/embed/-ljcRm7nS6o"
  frameborder="0"
  allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowfullscreen>
</iframe>
:::

::: {style="text-align: center;"}
[Point cloud and camera data collection demo](https://www.youtube.com/watch?v=-ljcRm7nS6o)
:::

## {data-menu-title="Algorithm flow chart" }

::: {style="font-size:50px;"}
![The overtaking vehicle detection and tracking algorithm](assets/images/algorithm-flowchart.png)
:::

## {data-menu-title="Segmented vehicles"}

:::: {.columns}
::: {.column}
![](assets/images/segmented-vehicle-1.png)
![](assets/images/segmented-vehicle-2.png)
:::
::: {.column}
![](assets/images/segmented-vehicle-3.png)
![](assets/images/segmented-vehicle-4.png)
:::
::::

::: {style="text-align:center;"}
Segmented overtaking vehicles with their 3D bounding boxes
:::

## Surrogate Safety Measures (1/3)
- **Passing distance**: the closest distance from the lidar to the car within the passing region
- **Passing duration**: the duration of time that a vehicle spends in the bicycle's passing region

:::: {.columns}
::: {.column}
![Begin of passing](assets/images/passing-phase-begin.png){height="220"}
:::
::: {.column}
![End of passing](assets/images/passing-phase-end.png){height="220"}
:::
::::

## Surrogate Safety Measures (2/3)
- **Bicycle speed**: Based on frame-to-frame registration with
  - RANSAC (Random Sample Consensus [@ransac])
  - ICP (Iterative Closest Point [@icp])

![](assets/images/icp-diagram.png){fig-align="center" height="340"}

## Surrogate Safety Measures (3/3)
- **Vehicle Speed**
  <!-- - **bicycle speed**  -->
  - **Relative vehicle speed** estimated using numerical differentiation of vehicle centroid locations across frames

  $$
  v(t)=\frac{x(t)-x(t-\Delta t)}{\Delta t}
  $$
  
$$
\text{vehicle speed} = \text{relative vehicle speed} + \text{bicycle speed}
$$

## Results

- 306 overtaking events from two trips
- 98 events had valid C3FT [@C3FT] readings (ground truth).
- Passing distance validation: MAE: 3.9 cm, MAPE: 2.5%

:::: {.columns}
::: {.column}
![](assets/images/c3ft-validation.png){height=270px}
:::
::: {.column}
![](assets/images/passing-dist.png){height=270px}
:::
::::

## {data-menu-title="Bicycle speed validation" }

::: {style="font-size:45px"}
![Validation of bicycle speed estimation based on GPS speed ground truth](assets/images/speed-validation-plot.png)
:::

## {data-menu-title="Result figures"}

:::: {.columns}
::: {.column}
![](assets/images/passing_distance_distribution_2.png)
![](assets/images/passing_duration_distribution_2.png)
:::
::: {.column}
![](assets/images/avg_speed_distribution_updated.png)
![](assets/images/speed_vs_passing_distance.png)
:::
::::

## Discussions

- Close passes (<1.5 m) occurred at both low and high speeds, showing risk is not speed-dependent.
- Passing duration revealed exposure over time; longer overtakes often due to vehicle congestion.
- Overtaking behavior depends on a combination of factors including: roadway design,  speed and traffic context.

## Future work

- Continued data collection across more roadway types and traffic conditions.
- Refine methods to improve vehicle detection reliability under challenging conditions like tilted bike and curved roadway conditions.
- Analyze vehicle trajectories to better understand other overtaking characteristics.
  - e.g., vehicle lateral movements, the *timing* selection to initiate an overtaking

## Acknowledgements

This material is based upon work supported by the National Science Foundation under [Award Number 2142757](https://www.nsf.gov/awardsearch/show-award?AWD_ID=2142757).

::: {style="font-size:40px; color:gray"}
"CAREER: Improving Bicycling Safety by Developing a Research Framework for Studying Driver-Bicyclist Interactions"
:::

![](assets/images/nsf-logo.png){fig-align="center" height="250"}

## References

::: {#refs}
:::

## Resources

- Slides URL & QR code
  - [fenggroup.org/trbam2026-lidar-bike](https://fenggroup.org/trbam2026-lidar-bike/)

  {{< qrcode https://fenggroup.org/trbam2026-lidar-bike/ qr1 width=320 height=320 >}}

- Public repositories
  - Data: [fenggroup.org/lidar-bike](https://fenggroup.org/lidar-bike/)
  - Code: [github.com/fenggroup/pointcloud-overtaking-vehicle-tracker](https://github.com/fenggroup/pointcloud-overtaking-vehicle-tracker)
- Contact: <fenggroup@umich.edu>
- Stay up to date: <https://fenggroup.org>
